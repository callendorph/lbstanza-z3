    ...
    for name, dep in graph.root.conanfile.dependencies.items():
        ...
        if (dep.package_type=='static-library' or dep.package_type=='shared-library'
            ...
            if incsrc.exists():
                ...
                for f in incsrc.iterdir():
                    graph.root.conanfile.output.trace(f"  copying dependency include {f} {incdst}")
                    copy2(f, incdst)
            if libsrc.exists():
                libdst.mkdir(parents=True, exist_ok=True)
                for f in libsrc.iterdir():
                    graph.root.conanfile.output.trace(f"  copying dependency lib {f} to {libdst}")
                    copy2(f, libdst)
